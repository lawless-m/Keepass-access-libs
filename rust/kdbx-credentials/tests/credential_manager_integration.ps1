...
#   2. The `lookup` example reads that credential from the Credential Manager,
#      opens the committed test.kdbx, and looks up an entry.
#
...
$DbPath = Join-Path $CrateDir 'tests/data/test.kdbx'
...
try {
    ...
    Write-Host '==> Running lookup example against the Credential Manager'
    $output = (& $Bin $DbPath $CredId $EntryPath 2>&1 | Out-String)
    $code = $LASTEXITCODE
