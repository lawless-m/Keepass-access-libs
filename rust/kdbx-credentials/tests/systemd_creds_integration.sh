#!/usr/bin/env bash
#
# End-to-end test of the Linux systemd-credentials secret store.
#
# It exercises the *real* mechanism the library relies on in production:
#
#   1. `systemd-creds encrypt` encrypts the master password to the host key.
#   2. `systemd-run` launches the `lookup` example as a transient *system*
...
echo "==> Building lookup example"
cargo build --quiet --manifest-path "$CRATE_DIR/Cargo.toml" --example lookup
BIN="$CRATE_DIR/target/debug/examples/lookup"
